  ...
  return (
    <div style={{ display: 'flex', height: '100vh', fontFamily: '-apple-system, BlinkMacSystemFont, sans-serif', background: '#f8f8f8' }}>
      ...
      <div style={{ width: 220, background: 'white', borderRight: '1px solid #eee', padding: '24px 0', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ padding: '9px 20px', fontSize: 13, color: '#555', cursor: 'pointer' }}>⊙ API Keys</div>
      ...
      <div style={{ flex: 1, overflow: 'auto' }}>
        <div style={{ background: 'white', borderBottom: '1px solid #eee', padding: '16px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 10 }}>
          <div>
            ...
            <div style={{ fontSize: 12, color: '#aaa', marginTop: 2 }}>Move to shortlist to trigger automated voice outreach</div>
          </div>
        ...
        <div style={{ padding: 24 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12 }}>
            {STATUSES.map(status => (
              <div key={status} style={{ background: 'white', border: '1px solid #eee', borderRadius: 12, overflow: 'hidden' }}>
                ...
                <div style={{ padding: 10, display: 'flex', flexDirection: 'column', gap: 8, minHeight: 100 }}>
                  ...
                  {byStatus(status).map(c => (
                    <div key={c.id} style={{ background: 'white', border: `1px solid #eee`, borderLeft: `2px solid ${STATUS_COLORS[status]}`, borderRadius: 8, padding: '10px 12px' }}>
                      ...
                      {status === 'applied' && (
                        <div
                          ...
                          style={{ fontSize: 11, color: '#534AB7', fontWeight: 500, marginTop: 8, cursor: 'pointer' }}
                        >
                  ...
                  {!loading && byStatus(status).length === 0 && (
                    <div style={{ fontSize: 12, color: '#ddd', padding: 8, textAlign: 'center' }}>Empty</div>
                  )}
                </div>
              </div>
            ))}
