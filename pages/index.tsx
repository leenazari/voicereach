...
export default function Dashboard() {
  ...
  function openEdit(candidate: Candidate) {
    ...
    setEditForm({ name: candidate.name || '', email: candidate.email || '', phone: candidate.phone || '', role_applied: candidate.role_applied || '', experience_summary: candidate.experience_summary || '', years_experience: candidate.years_experience?.toString() || '', job_title: candidate.job_title || '', job_salary: candidate.job_salary || '' })
    setShowEdit(true)
  }
  ...
  async function addCandidate() {
    ...
    const data = await res.json()
    if (data.candidate) { setShowAdd(false); setCvFile(null); setForm({ name: '', email: '', phone: '', role_applied: '', experience_summary: '', years_experience: '', job_title: '', job_salary: '' }); fetchCandidates(); notify('Candidate added successfully') }
    else notify('Error: ' + (data.error || 'Something went wrong'), 'error')
  }
        ...
        <div style={{ background: 'white', borderBottom: '1px solid #ebebeb', padding: '14px 28px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 10 }}>
          <div>
            <div style={{ fontSize: 16, fontWeight: 600, color: '#1a1a1a', letterSpacing: '-0.2px' }}>
              {activeTab === 'pipeline' ? 'Candidate Pipeline' : activeTab === 'candidates' ? 'All Candidates' : 'Analytics'}
            </div>
            <div style={{ fontSize: 12, color: '#aaa', marginTop: 1 }}>
              {activeTab === 'pipeline' ? 'Drag candidates between columns or click to shortlist' : activeTab === 'candidates' ? `${filteredAll.length} candidates total` : 'Performance overview'}
            </div>
          </div>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr style={{ background: '#fafafa', borderBottom: '1px solid #ebebeb' }}>
                    {['Name', 'Role', 'Email', 'Experience', 'Status', 'Actions'].map(h => (
                ...
                <tbody>
                  {loading ? (
                    <tr><td colSpan={6} style={{ padding: 24, textAlign: 'center', color: '#ccc', fontSize: 13 }}>Loading...</td></tr>
