    ...
    try {
      ...
      if (!res.ok || !data.token) {
        if (data.existing && data.token) {
          // Existing candidate who hasn't completed — let them continue
          setCandidateName(form.name.trim())
          setInterviewToken(data.token)
          setScreen('interview')
          return
        }
        setError(data.error === 'Interview already completed'
          ? 'You have already completed this interview. The hiring team will be in touch soon.'
          : data.error || 'Could not start interview — please try again')
        return
      }
      setCandidateName(form.name.trim())
      setInterviewToken(data.token)
