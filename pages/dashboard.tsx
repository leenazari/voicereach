...
export default function Dashboard() {
  ...
  async function findMatches(job: Job) {
    ...
    try {
      ...
      const data = await res.json()
      if (data.success) {
        await loadMatchResults(jobs.length > 0 ? jobs : [job])
      ...
      <div key={job.id} style={{ borderRadius: 10, overflow: 'hidden', border: '0.5px solid #e5e7eb', marginBottom: 2 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'stretch', minHeight: 68 }}>
          ...
          <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'flex-end', padding: '12px 14px', gap: 6, borderLeft: '0.5px solid #e5e7eb', background: 'white' }}>
            ...
            <button onClick={() => findMatches(job)} disabled={matchingJob === job.id}
              style={matchResults[job.id] ? { ...btnStyle('rgba(255,255,255,0)'), border: '1px solid #e5e7eb', color: '#374151' } : btnStyle('#f59e0b')}>
        ...
        <div style={{ background: 'white', padding: '8px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, borderTop: '0.5px solid rgba(0,0,0,0.06)' }}>
          ...
          {matchResults[job.id] && matchResults[job.id].length > 0 && (
            <span style={{ fontSize: 11, background: '#EEF2FF', color: '#4F46E5', padding: '2px 8px', borderRadius: 6, fontWeight: 500, flexShrink: 0 }}>
              {matchResults[job.id].filter(r => r.status === 'shortlist').length} matched
            </span>
          )}
        </div>
                    ...
                    <div style={{ padding: 8, display: 'flex', flexDirection: 'column' as const, gap: 8, minHeight: 80 }}>
                      {stageCandidates.length === 0 ? (
                        ...
                        <div key={m.candidate_id}
                          ...
                          {/* Score bar */}
                          <div style={{ height: 2, background: getScoreColor(m.match_score) }} />
                          <div style={{ padding: '10px 11px 9px' }}>
                            ...
                            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 8, marginBottom: 5 }}>
                              <div style={{ flex: 1, minWidth: 0 }}>
                                <div onClick={() => { const full = candidates.find(c => c.id === m.candidate_id); if (full) openProfile(full) }}
                                  style={{ fontSize: 12, fontWeight: 600, color: '#111827', cursor: 'pointer', marginBottom: 1 }}>{m.name}</div>
                                <div style={{ fontSize: 11, color: '#6b7280' }}>{m.role_applied}</div>
                                <div style={{ fontSize: 10, color: '#9ca3af', marginTop: 1 }}>
                                  {[m.last_employer, m.location].filter(Boolean).join(' · ')}
                                </div>
                              </div>
                              <div style={{ background: getScoreBg(m.match_score), borderRadius: 8, padding: '3px 8px', textAlign: 'center' as const, minWidth: 46, flexShrink: 0 }}>
                                <div style={{ fontSize: 20, fontWeight: 800, color: getScoreColor(m.match_score), lineHeight: 1 }}>{m.match_score}%</div>
                                <div style={{ fontSize: 8, color: getScoreColor(m.match_score), opacity: 0.7, marginTop: 1 }}>match</div>
                              </div>
                            </div>
                            {/* Keywords */}
                            ...
                            {/* Stage actions */}
                            {stage.id === 'matched' && (
                              <button onClick={() => moveCandidateStage(m.candidate_id, job.id, 'shortlisted')}
                                style={{ width: '100%', height: 28, background: '#4F46E5', color: 'white', border: 'none', borderRadius: 6, fontSize: 11, fontWeight: 500, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 4, marginBottom: 4 }}>
                                <svg width="10" height="10" viewBox="0 0 14 14" fill="none" stroke="white" strokeWidth="2" strokeLinecap="round"><path d="M7 1v12M1 7h12"/></svg>
                                Shortlist
                              </button>
                            )}
                            ...
                            {stage.id === 'invited' && (
                              <div style={{ fontSize: 9, color: '#7c3aed', background: '#f3e8ff', borderRadius: 5, padding: '5px 0', textAlign: 'center' as const, marginBottom: 5 }}>
                                <svg width="9" height="9" viewBox="0 0 14 14" fill="none" stroke="#7c3aed" strokeWidth="1.5" strokeLinecap="round" style={{ verticalAlign: 'middle', marginRight: 2 }}><path d="M7 2a3 3 0 013 3v1.5a3 3 0 01-6 0V5a3 3 0 013-3z"/><path d="M5 12c0 1.1.9 2 2 2s2-.9 2-2"/></svg>
                                Invite sent · awaiting interview
                              </div>
                            )}
                            {stage.id === 'interview_done' && (
                              <div style={{ display: 'flex', gap: 4, marginBottom: 4 }}>
                                <button onClick={() => moveCandidateStage(m.candidate_id, job.id, 'second_round')}
                                ...
                                <button onClick={() => moveCandidateStage(m.candidate_id, job.id, 'rejected')}
                            ...
                            {!['rejected', 'job_offer', 'interview_done'].includes(stage.id) && (
                              <button onClick={() => moveCandidateStage(m.candidate_id, job.id, 'rejected')}
    ...
    <div style={{ display: 'flex', height: '100vh', fontFamily: '-apple-system, BlinkMacSystemFont, sans-serif', background: '#f9fafb' }}>
      ...
      <div style={{ flex: 1, overflow: 'auto', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ padding: 24, flex: 1 }}>
          ...
          {/* ALL CANDIDATES */}
          {activeTab === 'candidates' && (() => {
            const filteredList = filteredAll
            const displayList = (() => {
              if (candidateFilter === 'interviewed') return filteredList.filter(c => (c as any).interview_completed_at)
              if (candidateFilter === 'shortlisted') return filteredList.filter(c => c.status === 'shortlisted' || c.status === 'voice_sent')
              if (candidateFilter === 'new') return filteredList.filter(c => c.status === 'new' || c.status === 'active')
              return filteredList
            })()
            ...
            const candidateRow = (c: typeof candidates[0]) => {
              ...
              return (
                <div key={c.id}
                  style={{ display: 'grid', gridTemplateColumns: cols, alignItems: 'center', background: 'white', border: '0.5px solid #e5e7eb', borderRadius: 10, padding: '10px 16px', marginBottom: 6, transition: 'border-color 0.15s', cursor: 'pointer' }}
                  onMouseEnter={e => (e.currentTarget.style.borderColor = '#4F46E5')}
                  onMouseLeave={e => (e.currentTarget.style.borderColor = '#e5e7eb')}
                >
                  {/* Name + status */}
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, minWidth: 0 }}>
                    <div style={{ width: 30, height: 30, borderRadius: 8, background: avatarBg, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 600, color: 'white', flexShrink: 0 }}>{initials}</div>
                    <div style={{ minWidth: 0 }}>
                      <div onClick={() => openProfile(c)} style={{ fontSize: 13, fontWeight: 600, color: '#111827', whiteSpace: 'nowrap' as const, overflow: 'hidden', textOverflow: 'ellipsis' }}>{c.name}</div>
                      <span style={{ fontSize: 9, padding: '1px 6px', borderRadius: 20, fontWeight: 500, background: STATUS_BG[c.status] || '#f3f4f6', color: STATUS_COLORS[c.status] || '#6b7280' }}>{STATUS_LABELS[c.status] || c.status}</span>
                    </div>
                  </div>
                  {/* Role */}
                  <div style={{ fontSize: 12, color: '#6b7280', whiteSpace: 'nowrap' as const, overflow: 'hidden', textOverflow: 'ellipsis', paddingRight: 8 }}>{c.role_applied || '—'}</div>
                  {/* Last employer */}
                  <div style={{ fontSize: 12, color: '#6b7280', whiteSpace: 'nowrap' as const, overflow: 'hidden', textOverflow: 'ellipsis', paddingRight: 8 }}>{(c as any).last_employer || '—'}</div>
                  {/* Location */}
                  <div style={{ fontSize: 12, color: '#6b7280', whiteSpace: 'nowrap' as const, overflow: 'hidden', textOverflow: 'ellipsis', paddingRight: 8 }}>{(c as any).location || '—'}</div>
                  {/* Exp */}
                  <div>
                  ...
                  <div style={{ display: 'flex', gap: 4, justifyContent: 'flex-end', alignItems: 'center' }}>
                    <button onClick={() => openProfile(c)} style={{ display: 'flex', alignItems: 'center', gap: 3, padding: '5px 8px', background: '#EEF2FF', color: '#4F46E5', border: 'none', borderRadius: 6, fontSize: 11, fontWeight: 500, cursor: 'pointer' }}>
                      <svg width="10" height="10" viewBox="0 0 14 14" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"><circle cx="7" cy="5" r="3"/><path d="M2 13c0-2.8 2.2-5 5-5s5 2.2 5 5"/></svg>
                      Profile
                    </button>
                    <button onClick={() => openJobModal(c)} style={{ display: 'flex', alignItems: 'center', gap: 3, padding: '5px 8px', background: '#0ea5e9', color: 'white', border: 'none', borderRadius: 6, fontSize: 11, fontWeight: 500, cursor: 'pointer' }}>
                      <svg width="10" height="10" viewBox="0 0 14 14" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"><path d="M1 7h12M8 3l4 4-4 4"/></svg>
                      {shortlisting === c.id ? '...' : 'Send'}
                    </button>
                    <button onClick={() => openEdit(c)} style={{ padding: '5px 8px', background: 'white', color: '#374151', border: '0.5px solid #e5e7eb', borderRadius: 6, fontSize: 11, fontWeight: 500, cursor: 'pointer' }}>Edit</button>
                    {c.voice_note_url && <button onClick={() => openPlayer(c)} style={{ padding: '5px 7px', background: '#dcfce7', color: '#15803d', border: 'none', borderRadius: 6, fontSize: 11, cursor: 'pointer' }}>▶</button>}
                    <button onClick={() => deleteCandidate(c)} style={{ display: 'flex', alignItems: 'center', padding: '5px 6px', background: 'white', color: '#ef4444', border: '0.5px solid #fecaca', borderRadius: 6, cursor: 'pointer' }}>
                      <svg width="10" height="10" viewBox="0 0 14 14" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"><path d="M2 4h10M5 4V2h4v2M5.5 7v4M8.5 7v4M3 4l.7 8a1 1 0 001 .9h4.6a1 1 0 001-.9L11 4"/></svg>
                    </button>
                  </div>
                </div>
              )
            ...
            return (
              <div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14, flexWrap: 'wrap' as const }}>
                  {(['all', 'interviewed', 'shortlisted', 'new'] as const).map(f => (
                    <button key={f} onClick={() => setCandidateFilter(f)}
                      style={{ padding: '6px 14px', borderRadius: 20, fontSize: 12, fontWeight: 500, cursor: 'pointer', border: candidateFilter === f ? 'none' : '1px solid #e5e7eb', background: candidateFilter === f ? '#4F46E5' : 'white', color: candidateFilter === f ? 'white' : '#6b7280' }}>
                      {f === 'all' ? 'All' : f === 'interviewed' ? 'Interviewed' : f === 'shortlisted' ? 'Shortlisted' : 'New'}
                    </button>
                  ))}
                ...
                {displayList.length === 0 ? (
                  <div style={{ background: 'white', borderRadius: 10, border: '0.5px solid #e5e7eb', padding: '48px 0', textAlign: 'center' as const, color: '#9ca3af', fontSize: 13 }}>No candidates match this filter</div>
                ) : (
                  <>
                    {colHeader}
                    {displayInterviewed.length > 0 && (
                      <>
                        ...
                        {displayInterviewed.map(c => candidateRow(c))}
                      </>
                    )}
                    {displayNotInterviewed.length > 0 && (
                      <>
                        ...
                        {displayNotInterviewed.map(c => candidateRow(c))}
                      </>
                    )}
                  </>
                )}
            ...
            <div style={{ padding: '24px 28px 20px', borderBottom: '0.5px solid #e5e7eb', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: 13, color: '#6b7280', marginTop: 2 }}>{jobModalCandidate.name} · {jobSendForm.jobTitle || 'No role selected'}</div>
                </div>
              </div>
              <button onClick={() => { setShowJobModal(false); setScriptPreview('') }}
            ...
            <div style={{ padding: '20px 28px 28px' }}>
              ...
              {jobs.filter(j => j.status === 'active').length > 0 && (
                <div style={{ marginBottom: 18 }}>
                  <label style={{ fontSize: 12, fontWeight: 600, color: '#374151', display: 'block', marginBottom: 6 }}>Role <span style={{ color: '#9ca3af', fontWeight: 400 }}>— select or enter manually below</span></label>
                  <select value={selectedJobId} onChange={e => handleJobSelect(e.target.value)}
                    style={{ width: '100%', padding: '10px 12px', border: '0.5px solid #e5e7eb', borderRadius: 8, fontSize: 13, outline: 'none', background: 'white', color: '#111827', cursor: 'pointer' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 20 }}>
                <div>
                  ...
                  <input type="text" value={jobSendForm.jobTitle} onChange={e => handleJobSendFormChange('jobTitle', e.target.value)}
                    placeholder="e.g. Senior Sales Executive"
                    style={{ width: '100%', padding: '10px 12px', border: '0.5px solid #e5e7eb', borderRadius: 8, fontSize: 13, outline: 'none', boxSizing: 'border-box' as const }} autoFocus />
                </div>
                <div>
                  <label style={{ fontSize: 12, fontWeight: 600, color: '#374151', display: 'block', marginBottom: 6 }}>Salary <span style={{ color: '#9ca3af', fontWeight: 400 }}>(optional)</span></label>
                  <input type="text" value={jobSendForm.jobSalary} onChange={e => handleJobSendFormChange('jobSalary', e.target.value)}
                    placeholder="e.g. £45,000"
                    style={{ width: '100%', padding: '10px 12px', border: '0.5px solid #e5e7eb', borderRadius: 8, fontSize: 13, outline: 'none', boxSizing: 'border-box' as const }} />
                </div>
              </div>
              ...
              <div style={{ marginBottom: 20 }}>
                ...
                <textarea
                  ...
                  rows={9}
                  placeholder={generatingPreview ? 'Generating script...' : 'Script will appear here once a job is selected'}
                  style={{ width: '100%', padding: '12px 14px', border: '0.5px solid #e5e7eb', borderRadius: 10, fontSize: 13, lineHeight: 1.7, resize: 'vertical', outline: 'none', background: generatingPreview ? '#f9fafb' : 'white', color: '#111827', boxSizing: 'border-box' as const, fontFamily: 'inherit' }}
                />
              </div>
              ...
              <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
                ...
                <button onClick={confirmShortlist} disabled={!jobSendForm.jobTitle || generatingPreview}
                  style={{ padding: '10px 24px', background: !jobSendForm.jobTitle || generatingPreview ? '#9ca3af' : '#4F46E5', color: 'white', border: 'none', borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: !jobSendForm.jobTitle || generatingPreview ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', gap: 7 }}>
                  ...
                  {generatingPreview ? 'Generating...' : 'Send voice note & invite'}
                </button>
              </div>
            </div>
          </div>
